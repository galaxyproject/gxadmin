...
_server_short_help="Gunicorn WSGI statistics"
_server_long_help="
	Various commands that contain the gunicorn service, such as active users... Please help to extend
"

server_active-users() { ## : Shows active users in last 10 minutes
	handle_help "$@" <<-EOF
		cu unique sorts IP adresses from 'GET /history/current_history_json' from last 10 minutes and prints it in influx line format
        EOF
