...
mutate_disassociate-training-roles() { ## [--commit] <-|training_id [training_id [...]]> : Disassociate training roles from groups.
	handle_help "$@" <<-EOF
		Disassociate training roles from their corresponding groups, e.g. after the training has ended. Useful
		in conjunction with 'gxadmin tsvquery tiaas-expired-trainings':

		$ PGDATABASE=tiaas gxadmin tsvquery tiaas-expired-trainings | gxadmin mutate disassociate-training-roles --commit -
	EOF
	...
	if [[ "$1" == "-" ]]; then
		# read training_ids from stdin
		training_ids=$(cat | paste -s -d' ')
	else
		# read from $@
		training_ids=$@;
	fi

	training_ids_string="'training-$(join_by "','training-" ${training_ids[@]})'"
	...
	read -r -d '' QUERY <<-EOF
		...
		WHERE
			...
			IN (
				...
				WHERE
					type = 'system'
					AND name in ($training_ids_string)
			)
	EOF
