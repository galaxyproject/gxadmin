...
query_wfsteps() { ## <wf-id> [--short-tool-id]: List the steps/tools in a workflow
	handle_help "$@" <<-EOF
	...
	assert_count_ge $# 1 "Missing wf-id"
	...
	read -r -d '' QUERY <<-EOF
		select type, $tool_id as tool_id, tool_version from workflow_step where workflow_id = '$1'
		order by order_index asc
	EOF
}

query_wfij(){ ## <wf-invocation-id> [--short-tool-id]: List the jobs in a workflow invocation
	handle_help "$@" <<-EOF
	...
	assert_count_ge $# 1 "Missing wf-invocation-id"

	# Use full tool id by default
	tool_id="j.tool_id"
	if [[ $2 = --short-tool-id ]]; then
		tool_id="regexp_replace(j.tool_id, '.*toolshed.*/repos/', '')"
	fi

	read -r -d '' QUERY <<-EOF
		SELECT
			ws.id, ws.workflow_id, ws.type, j.id, $tool_id as tool_id, j.tool_version, j.state, j.object_store_id, j.destination_id
		FROM
...
query_wfj() { ##  [--short-tool-id]: Query the new/running/unscheduled workflow jobs
	handle_help "$@" <<-EOF
