...
query_tool-usage() { ## query tool-usage: Counts of tool runs
	handle_help "$@" <<-EOF
		    $ gxadmin tool-usage
		                                    tool_id                                 | count
		    ------------------------------------------------------------------------+--------
...
query_training-memberof() { ## query training-memberof <username>: List trainings that a user is part of
	handle_help "$@" <<-EOF
	EOF
...
query_training-remove-member() { ## query training-remove-member <training> <username> [YESDOIT]: Remove a user from a training
	handle_help "$@" <<-EOF
	EOF
...
\n
EOF
	printf "$template" $results "$group_membership" "$role_membership" "$recent_jobs2" "$largest_histories"
}
