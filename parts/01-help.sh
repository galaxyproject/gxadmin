...
handle_help() {
	...
	for i in "$@"; do
		if [[ "$i" == "--help" || "$i" == "-h" || "$i" == "/h" ]]; then
			...
			man -l - <<-EOF
				.TH GXADMIN ${mode} ${query_name} "1" "TODO-DATE" "GXADMIN" "User Commands"
				.SH NAME
				...
				.SH COPYRIGHT
				Copyright \(co 2023 galaxyproject. License GPLv3+: GNU GPL version 3 or later. <https://gnu.org/licenses/gpl.html>.
				.br
